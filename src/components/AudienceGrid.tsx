...
export function AudienceGrid({ onRequestDemo }: AudienceGridProps) {
  return (
    <section id="industries" className="py-24 bg-brand-bg relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {AUDIENCES.map((item, idx) => {
            ...
            return (
              <div
                key={item.title}
                className="bg-brand-elevated border border-brand-border hover:border-brand-accent/40 rounded-xl p-6 sm:p-7 flex flex-col justify-between transition-all group"
              >
                <div>
                  <div className="w-10 h-10 rounded-lg bg-brand-surface border border-brand-border flex items-center justify-center text-brand-accent mb-5 group-hover:border-brand-accent/40 transition-colors">
                    <Icon className="w-5 h-5" />
                  </div>

                  <h3 className="font-serif text-xl font-bold text-brand-text mb-1 group-hover:text-brand-accent-light transition-colors">
                    {item.title}
                  </h3>
                  <div className="text-xs font-mono text-brand-accent mb-3">
                    {item.subtitle}
                  </div>
                ...
                <div className="pt-4 border-t border-brand-border/60">
                  <ul className="space-y-2">
                    {item.points.map((pt, pIdx) => (
                      <li key={pIdx} className="flex items-center gap-2 text-xs text-brand-secondary">
          ...
          {/* Action CTA Card */}
          <div className="bg-brand-surface/40 border border-dashed border-brand-strong-border rounded-xl p-6 sm:p-7 flex flex-col justify-between">
            <div>
