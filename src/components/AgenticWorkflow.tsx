...
export function AgenticWorkflow({ onRequestDemo }: AgenticWorkflowProps) {
  ...
  return (
    <section id="agentic" className="py-24 bg-brand-bg relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-brand-elevated border border-brand-border rounded-xl p-6 sm:p-8 mb-12">
          ...
          <div className="flex items-center justify-between pb-6 border-b border-brand-border mb-8">
            <div className="flex items-center gap-2 text-xs font-mono text-brand-text">
              ...
              <span>AGENTIC PIPELINE // HUMAN-IN-THE-LOOP ARCHITECTURE</span>
            </div>
            <span className="text-[10px] font-mono bg-brand-surface px-2.5 py-1 rounded text-brand-secondary border border-brand-border">
            ...
            {steps.map((step, idx) => {
              ...
              return (
                <div key={step.num} className="relative flex flex-col justify-between">
                  <div
                    className={`h-full p-4 rounded-lg border transition-all ${
                      step.isHumanGate
                  ...
                  >
                    ...
                    {step.isHumanGate && (
                      <div className="mt-3 pt-2 border-t border-brand-accent/30 text-[9px] font-mono text-brand-accent uppercase font-bold">
                        ★ Human Decision Gate
                      </div>
                    )}
                  </div>
                  ...
                  {idx < steps.length - 1 && (
                    <div className="hidden md:flex absolute top-1/2 -right-3 -translate-y-1/2 z-10 text-brand-border">
          ...
          <div className="mt-8 pt-6 border-t border-brand-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs text-brand-secondary">
            ...
            <button
              ...
              className="text-xs text-brand-accent hover:underline font-mono whitespace-nowrap"
            >
              Learn about Agentic Integrations →
            </button>
          </div>
